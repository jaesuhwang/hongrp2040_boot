...
static uint32_t reset_boot_mode = MODE_BIT_UPDATE;
// static uint32_t reset_boot_mode = MODE_BIT_BOOT;
...
{
  resetSetBootMode(MODE_BIT_BOOT);
  NVIC_SystemReset();
}
...
const char *resetGetBootModeMsg(void)
{
  ...
  switch(reset_mode)
  {
    ...
    //   break;
    case  MODE_BIT_BOOT:
      ret_str = BOOT_MSG_STR(MODE_BIT_BOOT);
    break;

    case  MODE_BIT_UPDATE:
      ret_str = BOOT_MSG_STR(  MODE_BIT_UPDATE);
    break;
  }
